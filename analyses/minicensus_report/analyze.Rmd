...
left <- tza$minicensus_main %>% dplyr::select(hh_id, instance_id_psql = instance_id) %>% mutate(in_databrew = TRUE)
right <- readxl::read_excel('data/Minicensus_clean_LocalDB.xlsx') %>% dplyr::select(hh_id, instance_id_imani = instanceid) %>%
  mutate(in_ihi = TRUE)
