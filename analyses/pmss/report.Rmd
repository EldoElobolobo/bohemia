---
title: " MALARIA PASSIVE SURVEILLANCE SYSTEM - MONITORING REPORTS"
output:
  html_document:
    df_print: paged
...
ids <- c('malariaregisterbaseline1moz',  'malariaregisterbaseline2moz', 'malariaregisterpost1moz', 'malariaregisterpost2moz', 'passivemalariasurveillancemoz' ) 
...
#tbl_names1<-c(10, 11, 12)
#knitr::kable(all_data[ ,tbl_names1])
knitr::kable(head(tbl1), col.names = c("Health facility name", "Date, Month", "No. PMSS questionnaire from Health Facility ", "No. PMSS questionnaire from APEs"  ),  "pipe")
```
...
knitr::kable(head(tbl2), col.names = c("Health facility", "Date, Month", "No. Malaria register completion Baseline Reports (Phase 1)", "No. of intervention Phase 2 forms required", "No. Malaria register completion Post-Baseline Reports (Phase 2)"),  "pipe")
```
...
tbl3_n <- rbind(tbl3, c("Total", colSums(tbl3[,2:6])))
knitr::kable(head(tbl3_n), col.names = c("Health facility", "No. PMSS questionnaire from Health Facility", "No. PMSS questionnaire from APEs", "No. Malaria register completion Baseline Reports (Phase 1)", "No. of intervention Phase 2 forms required", "No. Malaria register completion Post-Baseline Reports (Phase 2)"),  "pipe")
...
library("ggplot2")
ggplot(tbl1, aes(Health_facility,one, label = Health_facility)) + 
     geom_bar(stat="identity") + 
     facet_wrap(~Month, scales="free") +
  labs(y="Number of cases", x= "Health facility", title = "No. PMSS questionnaire from Health Facility")

ggplot(tbl1, aes(Health_facility,ape_count, label = Health_facility)) + 
     geom_bar(stat="identity") + 
     facet_wrap(~Month, scales="free") +
  labs(y="Number of cases", x= "Health facility",title = "No. PMSS questionnaire from APEs")

ggplot(tbl2, aes(hf_name, phase1, label = hf_name)) + 
     geom_bar(stat="identity") + 
     facet_wrap(~start_time, scales="free") +
  labs(y="Number of cases", x= "Health facility", title  = "No. Malaria register completion Baseline Reports (Phase 1)")

ggplot(tbl2, aes(hf_name, result, label = hf_name)) + 
     geom_bar(stat="identity") + 
     facet_wrap(~start_time, scales="free") +
  labs(y="Number of cases", x= "Health facility", title  = "No. of intervention Phase 2 forms required")

ggplot(tbl2, aes(hf_name, phase2, label = hf_name)) + 
     geom_bar(stat="identity") + 
     facet_wrap(~start_time, scales="free") +
  labs(y="Number of cases", x= "Health facility", title  = "No. Malaria register completion Post-Baseline Reports (Phase 2)")
